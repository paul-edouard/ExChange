...
IObjectiveFunction<double[]> {
	...
	public ParabolicSARObjFunc(String field, double penalty,
			LinkedList<HistoricalPoint> pList, double maxNumberOfDays,
			double maxBandFactor, double maxProfit) {
		this.field = field;
	...
	public double compute(double[] x, Random r) {
		...
		// Init the series
		movingAverageUpperSeries = new XYSeries(
				BollingerBand_MovingAverage_Upper);
		movingAverageLowerSeries = new XYSeries(
				BollingerBand_MovingAverage_Lower);
		...
		// get the Day list
		LinkedList<HistoricalPoint> pList = HistoricalData.getPointsFromPeriod(
				n_period, this.noneZeroHisList);
		if (pList.isEmpty() || pList.size() <= numberOfDays) {
			return 0;
		}
		...
		for (int i = (int) numberOfDays; i < pList.size(); i++) {
			...
			if ((i - numberOfDays + 1) >= 0) {
				...
				for (int j = i - (int) numberOfDaysUpper + 1; j <= i; j++) {
					stdDevUpperMax += Math.pow(pList.get(j).get(upperField)
							- avgUpperMax, 2);
					stdDevUpperMin += Math.pow(pList.get(j).get(lowerField)
							- avgUpperMin, 2);
				}
				stdDevUpperMax = Math.sqrt(stdDevUpperMax / numberOfDaysUpper);
				stdDevUpperMin = Math.sqrt(stdDevUpperMin / numberOfDaysUpper);

				double bandUpperMax = avgUpperMax
						+ (bandFactorUpper + bandDamperUpper) * stdDevUpperMax;
				double bandUpperMin = avgUpperMin
						+ (bandFactorUpper - bandDamperUpper) * stdDevUpperMin;

				movingAverageUpperSeries.add((i - numberOfDays + 1),
						avgUpperMax);
				...
				for (int j = i - (int) numberOfDaysLower + 1; j <= i; j++) {
					stdDevLowerMax += Math.pow(pList.get(j).get(upperField)
							- avgLowerMax, 2);
					stdDevLowerMin += Math.pow(pList.get(j).get(lowerField)
							- avgLowerMin, 2);
				}
				stdDevLowerMax = Math.sqrt(stdDevLowerMax / numberOfDaysLower);
				stdDevLowerMin = Math.sqrt(stdDevLowerMin / numberOfDaysLower);

				double bandLowerMax = avgLowerMax
						- (bandFactorLower - bandDamperLower) * stdDevLowerMax;
				double bandLowerMin = avgLowerMin
						- (bandFactorLower + bandDamperLower) * stdDevLowerMin;

				movingAverageLowerSeries.add((i - numberOfDays + 1),
						avgLowerMax);
				...
				if (!bought) {
					...
					if (pList.get(i).get(lowerField) < bandLowerMin) {
						...
						buySignalSeries.add((i - numberOfDays + 1),
								lastBuyPrice);
					}
					// The curve goes upper
					else if (isLower
							&& pList.get(i).get(upperField) > bandLowerMax) {
				...
				// Sell Signal
					...
					if (pList.get(i).get(upperField) > bandUpperMax) {
						...
						sellSignalSeries.add((i - numberOfDays + 1),
								bandUpperMax);
				...
				// Set Lower and Upper
				isUpper = (pList.get(i).get(upperField) < bandUpperMax && pList
						.get(i).get(lowerField) > bandUpperMin);

				isLower = (pList.get(i).get(upperField) < bandLowerMax && pList
						.get(i).get(lowerField) > bandLowerMin);

				profitSeries.add((i - numberOfDays + 1),
						profit / pList.get((int) numberOfDays).get(field));
				...
				/*
				 * private boolean daySellUpLimitIsActivated=false; private
				 * boolean daySellDownLimitIsActivated=false;
				 * 
				 * private boolean dayBuyUpLimitIsActivated=false; private
				 * boolean dayBuyDownLimitIsActivated=false;
				 */
