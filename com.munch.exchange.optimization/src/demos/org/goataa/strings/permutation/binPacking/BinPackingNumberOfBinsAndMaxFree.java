 ...
 * @author Thomas Weise
 */
public class BinPackingNumberOfBinsAndMaxFree extends BinPackingInstance
   ...
   *          the object sizes
   */
  public BinPackingNumberOfBinsAndMaxFree(final int pb, final int[] pa) {
    super(pb, pa, -1, "f4"); //$NON-NLS-1$
  }
   ...
   */
  public BinPackingNumberOfBinsAndMaxFree(final BinPackingInstance bpi) {
    super(bpi, "f4");//$NON-NLS-1$
  }
  ...
  public final double compute(final int[] x, final Random r) {
    int k, maxFree, maxFreeT, free, size, i;
    ...
    // We want to minimize the number of bins and maximize the maximum free
    // space (now stored in "maxFree"). k is a natural number and should be
    // the dominating factor. We can add a value in [0,1) to represent the
    // free space. This factor should be the smaller, the larger the free
    // space is. By choosing 1/(1+maxFreeT*maxFree), we can realize this.
    // Amongst two solutions which have the same number of bins, now the
    // one with more free space will be preferred.
    return k + (1d / (1d + (maxFreeT * maxFree)));
  }
}