...
public class KeepBestSGEA<G, X> extends  EABase<G, X> {
	  ...
	  public static final <G, X> Individual<G, X> evolutionaryAlgorithm(
	    ...
	    List<Individual<G, X>> children=new LinkedList<Individual<G, X>>();
	    ...
	    for (;;) {
	      ...
	      // create cr*mps individuals with crossover
	      k = (int) (0.5 + (cr * mps));
	      for (j = 0; (j < k) ; j++) {
	        p = new Individual<G, X>();
	      ...
	      for( j=0;j<ps;j++){
	    	  if(bestInd.isEmpty()){
    			  bestInd.addFirst(pop[j]);continue;
    		  }
    		  int insetPos=0;
    		  for(Individual<G, X> ind:bestInd){
    			  if(pop[j].v<ind.v)break;
    			  insetPos++;
    		  }
	  ...
	  public void call(final Random r, final ITerminationCriterion term,
	    ...
	    result.add(KeepBestSGEA.evolutionaryAlgorithm(//
